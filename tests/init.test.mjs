...
test("initProject initializes an empty project with default sources and skills", async () => {
  await withProject(async (projectPath, defaults, cacheRoot) => {
    ...
    const productDesignerRole = lockfile.roles.find((role) => role.activeName === "product-designer");
    const requirementsEngineerRole = lockfile.roles.find((role) => role.activeName === "requirements-engineer");
    const technicalArchitectRole = lockfile.roles.find((role) => role.activeName === "technical-architect");
    const securityEngineerRole = lockfile.roles.find((role) => role.activeName === "security-engineer");
    ...
    assert.equal(productStrategistRole.sourcePath, "roles/project-dev/product-strategist");
    ...
    assert.equal(productStrategistRole.packagePath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/product-strategist");
    assert.equal(productStrategistRole.activationPath, ".agents/roles/product-strategist");
    assert.ok(productDesignerRole);
    assert.equal(productDesignerRole.source, "aix");
    assert.equal(productDesignerRole.sourcePath, "roles/project-dev/product-designer");
    assert.equal(productDesignerRole.requested, false);
    assert.deepEqual(productDesignerRole.owner, { kind: "workflow", name: "design-plan-execute" });
    assert.equal(productDesignerRole.packagePath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/product-designer");
    assert.equal(productDesignerRole.activationPath, ".agents/roles/product-designer");
    assert.ok(requirementsEngineerRole);
    assert.equal(requirementsEngineerRole.source, "aix");
    assert.equal(requirementsEngineerRole.sourcePath, "roles/project-dev/requirements-engineer");
    assert.equal(requirementsEngineerRole.requested, false);
    assert.deepEqual(requirementsEngineerRole.owner, { kind: "workflow", name: "design-plan-execute" });
    assert.equal(requirementsEngineerRole.packagePath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/requirements-engineer");
    assert.equal(requirementsEngineerRole.activationPath, ".agents/roles/requirements-engineer");
    assert.ok(technicalArchitectRole);
    assert.equal(technicalArchitectRole.source, "aix");
    assert.equal(technicalArchitectRole.sourcePath, "roles/project-dev/technical-architect");
    assert.equal(technicalArchitectRole.requested, false);
    assert.deepEqual(technicalArchitectRole.owner, { kind: "workflow", name: "design-plan-execute" });
    assert.equal(technicalArchitectRole.packagePath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/technical-architect");
    assert.equal(technicalArchitectRole.activationPath, ".agents/roles/technical-architect");
    assert.ok(securityEngineerRole);
    assert.equal(securityEngineerRole.source, "aix");
    assert.equal(securityEngineerRole.sourcePath, "roles/project-dev/security-engineer");
    assert.equal(securityEngineerRole.requested, false);
    assert.deepEqual(securityEngineerRole.owner, { kind: "workflow", name: "design-plan-execute" });
    assert.equal(securityEngineerRole.packagePath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/security-engineer");
    assert.equal(securityEngineerRole.activationPath, ".agents/roles/security-engineer");
    assert.ok(uxWriterRole);
    assert.equal(uxWriterRole.source, "aix");
    assert.equal(uxWriterRole.sourcePath, "roles/project-dev/ux-writer");
    assert.equal(uxWriterRole.requested, false);
    assert.deepEqual(uxWriterRole.owner, { kind: "workflow", name: "design-plan-execute" });
    assert.equal(uxWriterRole.packagePath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/ux-writer");
    assert.equal(uxWriterRole.activationPath, ".agents/roles/ux-writer");
    assert.ok(qualityEngineerRole);
    assert.equal(qualityEngineerRole.source, "aix");
    assert.equal(qualityEngineerRole.sourcePath, "roles/project-dev/quality-engineer");
    assert.equal(qualityEngineerRole.requested, false);
    assert.deepEqual(qualityEngineerRole.owner, { kind: "workflow", name: "design-plan-execute" });
    assert.equal(qualityEngineerRole.packagePath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/quality-engineer");
    assert.equal(qualityEngineerRole.activationPath, ".agents/roles/quality-engineer");
    assert.ok(documentationSpecialistRole);
    assert.equal(documentationSpecialistRole.source, "aix");
    assert.equal(documentationSpecialistRole.sourcePath, "roles/project-dev/documentation-specialist");
    assert.equal(documentationSpecialistRole.requested, false);
    assert.deepEqual(documentationSpecialistRole.owner, { kind: "workflow", name: "design-plan-execute" });
    assert.equal(documentationSpecialistRole.packagePath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/documentation-specialist");
    assert.equal(documentationSpecialistRole.activationPath, ".agents/roles/documentation-specialist");
    assert.ok(implementationEngineerRole);
    assert.equal(implementationEngineerRole.source, "aix");
    assert.equal(implementationEngineerRole.sourcePath, "roles/project-dev/implementation-engineer");
    ...
    assert.equal(implementationEngineerRole.packagePath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/implementation-engineer");
    assert.equal(implementationEngineerRole.activationPath, ".agents/roles/implementation-engineer");
    ...
    assert.ok(existsSync(join(projectPath, ".agents/packages/workflows/aix/design-plan-execute/templates/sections/security-review.md")));
    ...
    assert.ok(existsSync(join(projectPath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/product-strategist/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/roles/product-strategist/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/product-designer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/roles/product-designer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/requirements-engineer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/roles/requirements-engineer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/technical-architect/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/roles/technical-architect/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/security-engineer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/roles/security-engineer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/ux-writer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/roles/ux-writer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/quality-engineer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/roles/quality-engineer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/documentation-specialist/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/roles/documentation-specialist/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/packages/workflows/aix/design-plan-execute/roles/project-dev/implementation-engineer/ROLE.md")));
    assert.ok(existsSync(join(projectPath, ".agents/roles/implementation-engineer/ROLE.md")));
