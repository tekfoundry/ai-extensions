...
test("run diff reports source changes without mutating package files or the lockfile", async () => {
...
test("run diff reports no changes when locked packages match the source", async () => {
...
test("run diff with a target reports only the matching locked skill", async () => {
  await withProject(async (projectRoot, gitSource) => {
    assert.equal(run(["activate", "skill", "fixture/skills/demo"]).exitCode, 0);
    ...
    const result = run(["diff", "fixture/skills/demo"]);
...
test("run diff with a target fails for unknown locked skills", async () => {
  await withProject(async () => {
    const result = run(["diff", "fixture/skills/missing"]);
...
test("run diff reports no changes when no skills are locked", async () => {
  await withProject(async () => {
    const result = run(["diff"]);
