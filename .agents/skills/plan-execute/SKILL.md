---
name: plan-execute
description: Orchestrate execution of an active implementation plan across phases. Use when the user asks Codex to execute, continue, or complete an entire active plan; repeatedly route bounded phase work through `phase-execute` until completion or a stop condition.
---
...
1. Resolve the active plan explicitly, or infer it only when exactly one active
   plan matches the user request. Stop if the plan is backlog or completed.
